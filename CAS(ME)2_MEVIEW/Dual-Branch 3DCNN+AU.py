...
def process_video_frames(video_path, image_rows, image_columns, image_depth, output_folder, AU_numbers_dict):
    ...
    for frame_idx in range(0, total_frames):
        ...
        prev_frame = image
    # 选取变化最大的**帧的索引
    top_indices = np.argsort(frame_diffs)[-8:]
    # 按索引排序
...
# 参数设置
image_rows, image_columns, image_depth = 128, 128, 8
output_folder = r'C:\Users\user\Desktop\micro-expression-recognition-master\CASME\training_frames'
negativepath = 'C:/Users/user/Desktop/micro-expression-recognition-master/CASME/data(CAS(ME)2)/negative/'
positivepath = 'C:/Users/user/Desktop/micro-expression-recognition-master/CASME/data(CAS(ME)2)/positive/'
surprisepath = 'C:/Users/user/Desktop/micro-expression-recognition-master/CASME/data(CAS(ME)2)/surprise/'
otherspath = 'C:/Users/user/Desktop/micro-expression-recognition-master/CASME/data(CAS(ME)2)/others/'
# 指定 Excel 檔案的路徑
excel_file_path = r"C:\Users\user\Desktop\micro-expression-recognition-master\CASME\CAS(ME)^2.xlsx"
df = pd.read_excel(excel_file_path, sheet_name='CASFEcode_final')
...
# 合并所有视频
training_list = neg_videos + pos_videos + sur_videos + oth_videos
training_list2 = neg_videos_local + pos_videos_local + sur_videos_local + oth_videos_local
video_list = neg_list + pos_list + sur_list + oth_list
...
traininglabels[len(neg_videos) + len(pos_videos):len(neg_videos) + len(pos_videos) + len(sur_videos)] = 2
...
for train_index, test_index in logo.split(X, y, groups=subjects):
    ...
    combined_output = LeakyReLU(alpha=0.2)(combined_output)
    output_layer_final = Dense(4, activation='softmax')(combined_output)
    # 建立整合後的模型
    model = Model(inputs=[input_layer, input_layer2], outputs=output_layer_final)
    class_weights = [0.2, 0.3, 0.3, 0.2]
