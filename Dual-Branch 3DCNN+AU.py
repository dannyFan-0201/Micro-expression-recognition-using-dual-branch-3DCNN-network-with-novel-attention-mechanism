...
# from test import extract_AU_numbers, GraphConvolution, GCN
...
def process_video_frames(video_path, image_rows, image_columns, image_depth, output_folder, AU_numbers_dict, onset_dict, apex_dict, offset_dict):
    # 初始化人脸检测器和关键点预测器
    ...
    one_value = onset_dict.get(video_name, 0)
    apex_value = apex_dict.get(video_name, 0)
    off_value = offset_dict.get(video_name, 0)
    # target_frames = 30
    # 生成线性插值的索引
    # frame_indices = np.linspace(0, total_frames - 1, target_frames, dtype=int)
    # frame_indices = np.linspace(0, off_value-1, target_frames, dtype=int)
    # 生成ApexFrame前后各15帧
    # frame_indices1 = list(range(one_value, one_value + 5))
    # frame_indices2 = list(range(apex_value - 10, apex_value + 10))
    # frame_indices3 = list(range(off_value - 6, off_value - 1))
    # frame_indices = frame_indices1 + frame_indices2 + frame_indices3
    # framerange = [x + 0 for x in range(30)]
    # 讀取所有幀，計算差異並存儲
    prev_frame = None
    for frame_idx in range(0, off_value - 1):
...
def process_videos_in_path(path, label, image_rows, image_columns, image_depth, output_folder, AU_numbers_dict, onset_dict, apex_dict, offset_dict):
    training_list = []
    training_list2 = []
    video_list = os.listdir(path)
    for video in video_list:
        video_path = os.path.join(path, video)
        videoarray, local_videoarray = process_video_frames(video_path, image_rows, image_columns, image_depth, output_folder, AU_numbers_dict, onset_dict, apex_dict, offset_dict)
...
output_folder = r'C:\Users\user\Desktop\micro-expression-recognition-master\CASME2\training_frames'
negativepath = 'C:/Users/user/Desktop/micro-expression-recognition-master/data/negative/'
positivepath = 'C:/Users/user/Desktop/micro-expression-recognition-master/data/positive/'
surprisepath = 'C:/Users/user/Desktop/micro-expression-recognition-master/data/surprise/'
# 指定 Excel 檔案的路徑
excel_file_path = r"C:\Users\user\Desktop\micro-expression-recognition-master\CASME2-coding-20140508.xlsx"
...
AU_numbers_dict = dict(zip(df['Filename'], AU_list))
print("Extracted AU numbers list:", AU_numbers_dict)
onset_frames = df['OnsetFrame'].dropna().astype(int).values
onset_dict = dict(zip(df['Filename'], onset_frames))
apex_frames = df['ApexFrame'].dropna().astype(int).values
apex_dict = dict(zip(df['Filename'], apex_frames))
offset_frames = df['OffsetFrame'].dropna().astype(int).values
offset_dict = dict(zip(df['Filename'], offset_frames))

diff_dict = {filename: offset_dict[filename] - onset_dict[filename] for filename in onset_dict if filename in offset_dict}
# # 将结果保存到Excel文件
# output_file_path = r'C:\Users\user\Desktop\micro-expression-recognition-master\CASME2-test.xlsx'  # 替换为你希望保存的文件路径
# diff_df = pd.DataFrame(list(diff_dict.items()), columns=['Filename', 'Offset - Onset'])
# diff_df.to_excel(output_file_path, index=False)
# # 打印结果
# for filename, diff in diff_dict.items():
#     print(f"Filename: {filename}, Offset - Onset: {diff}")

# 处理每个类别的视频
neg_videos, neg_videos_local, neg_list = process_videos_in_path(negativepath, 0, image_rows, image_columns, image_depth, output_folder, AU_numbers_dict, onset_dict, apex_dict, offset_dict)
pos_videos, pos_videos_local, pos_list = process_videos_in_path(positivepath, 1, image_rows, image_columns, image_depth, output_folder, AU_numbers_dict, onset_dict, apex_dict, offset_dict)
sur_videos, sur_videos_local, sur_list = process_videos_in_path(surprisepath, 2, image_rows, image_columns, image_depth, output_folder, AU_numbers_dict, onset_dict, apex_dict, offset_dict)
